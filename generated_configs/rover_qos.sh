...
for i in eth0.900 eth0.24; do
    ...
    tc filter add dev $i protocol ip parent 1:0 prio 3 u32 match ip src 192.168.2.0/24 flowid 1:1
    tc filter add dev $i protocol ip parent 1:0 prio 4 u32 match ip src 192.168.3.0/24 flowid 1:1

    # BAND 3 (LOWEST PRIORITY - Cameras/Science - Dropped if congested)
    tc filter add dev $i protocol ip parent 1:0 prio 5 u32 match ip src 192.168.4.0/24 flowid 1:3
done