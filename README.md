...
* **If using Raspberry Pi OS Bookworm (Pi 5) / NetworkManager:**
  You need to tell NetworkManager to ignore your transit and radio interfaces so FRR can take full control of them. On the Pis, run:

  ```bash
  sudo nano /etc/NetworkManager/conf.d/99-unmanaged-devices.conf
  ```

  Paste this inside the file to make NetworkManager blind to your router interfaces (you may need to change the name of the interfaces):

  ```ini
  [keyfile]
  unmanaged-devices=interface-name:eth0.99;interface-name:eth0.900;interface-name:eth0.24
  
  ```

  Save the file (Ctrl+O, Enter, Ctrl+X), then restart the networking stack and FRR:
